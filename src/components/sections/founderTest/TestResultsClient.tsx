...
import type { TestResult, PersonalityType } from "@/lib/hooks/useFounderTestEngine";
...
const P_CONFIG: Record<PersonalityType, { icon: string; accent: string }> = {
  blindfolded_founder:   { icon: "◎", accent: "#DC2626" },
  hustler_rough_numbers: { icon: "◇", accent: "#D97706" },
  structured_operator:   { icon: "◈", accent: "#D97706" },
  finance_savvy_builder: { icon: "△", accent: "#059669" },
  investor_ready_ceo:    { icon: "◆", accent: "#1D4ED8" },
};
...
export function TestResultsClient({ result }: Props) {
  const cfg       = P_CONFIG[result.personalityType];
  const radarData = result.scores.map(s => ({ subject: s.label, score: s.percentage, fullMark: 100 }));
  ...
  return (
    <Box sx={{ minHeight: "100vh", background: T.bg, fontFamily: SANS }}>
      ...
      <Box sx={{ display: "flex", flexDirection: { xs: "column", lg: "row" }, minHeight: { lg: "85vh" } }}>
          ...
          <Box sx={{ position: "relative", zIndex: 1 }}>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.55, ease: EASE }}>
              <Typography sx={{ fontFamily: MONO, fontSize: "0.54rem", letterSpacing: "0.2em", color: "#A5B4FC", textTransform: "uppercase", mb: 2 }}>
                Financial Personality Results
              </Typography>
              <Typography sx={{ fontFamily: SANS, fontWeight: 800, fontSize: { xs: "1.875rem", md: "2.625rem" }, color: T.white, letterSpacing: "-0.03em", lineHeight: 1.05, mb: 1 }}>
        ...
        {/* Right — radar */}
        <Box sx={{ flex: 1, background: T.white, display: "flex", alignItems: "center", justifyContent: "center", p: { xs: "40px 24px", md: "56px 56px" }, borderLeft: { lg: `1px solid ${T.border}` } }}>
      ...
      <Container maxWidth="xl" sx={{ py: { xs: 5, md: 7 } }}>
        ...
        {/* Founder message */}
        <Section delay={0.05}>
          <Box sx={{ mb: 3, p: { xs: 2.5, md: 3.5 }, borderRadius: "10px", background: T.white, border: `1px solid ${T.border}`, borderLeft: "3px solid", borderLeftColor: cfg.accent, boxShadow: "0 2px 12px rgba(10,10,20,0.05)" }}>
            <Typography sx={{ fontFamily: MONO, fontSize: "0.5rem", letterSpacing: "0.16em", color: T.inkFaint, textTransform: "uppercase", mb: 1.25 }}>Message for you</Typography>
            <Typography sx={{ fontFamily: SANS, fontSize: "0.9375rem", color: T.inkMid, lineHeight: 1.8 }}>{result.message}</Typography>
          </Box>
        </Section>
        ...
        {/* CTA */}
        <Section delay={0.4}>
          <Box sx={{ background: T.darkBg, borderRadius: "12px", p: { xs: 3.5, md: 6 }, position: "relative", overflow: "hidden" }}>
            <Box sx={{ position: "absolute", inset: 0, pointerEvents: "none",
            ...
            <Box sx={{ position: "relative", zIndex: 1, maxWidth: 560 }}>
              <Typography sx={{ fontFamily: MONO, fontSize: "0.54rem", letterSpacing: "0.18em", color: "#A5B4FC", textTransform: "uppercase", mb: 2 }}>Next Steps</Typography>
              ...
              <Typography sx={{ fontFamily: SANS, fontWeight: 800, fontSize: { xs: "1.75rem", md: "2.375rem" }, letterSpacing: "-0.03em", lineHeight: 1.05, mb: 2, background: T.grad, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>
                your insights?
              </Typography>
              <Typography sx={{ fontFamily: SANS, fontSize: "0.9375rem", color: T.darkMuted, lineHeight: 1.75, mb: 4, maxWidth: 440 }}>
                Book a free 30-minute consultation and build a financial roadmap tailored to your personality type.
              </Typography>
              <Box sx={{ display: "flex", gap: 1.5, flexWrap: "wrap" }}>
                <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
                  <Box component="a" href={process.env.NEXT_PUBLIC_CALENDLY_URL} target="_blank" rel="noopener noreferrer" sx={{ display: "inline-flex", alignItems: "center", gap: 1.25, px: 2.5, py: "12px", borderRadius: "8px", background: T.btn, textDecoration: "none", boxShadow: "0 4px 20px rgba(124,58,237,0.3)" }}>
                ...
                <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
                  <Box component={Link} href="/templates" sx={{ display: "inline-flex", alignItems: "center", gap: 1.25, px: 2.5, py: "12px", borderRadius: "8px", border: `1px solid ${T.darkBorder}`, textDecoration: "none", transition: "border-color 0.15s, background 0.15s", "&:hover": { borderColor: "rgba(255,255,255,0.22)", background: "rgba(255,255,255,0.04)" } }}>
