...
/* ── Inline schema (no external dep) ── */
const founderTestContactSchema = z.object({
...
type FounderTestContactValues = z.infer<typeof founderTestContactSchema>;
      ...
      {helperText && (
        <Typography
          sx={{
            fontFamily: SANS,
            fontSize: "0.7rem",
            color: error ? T.error : T.inkFaint,
            lineHeight: 1.4,
          }}
        >
          {helperText}
...
export function TestContactScreen({ onSubmit, onBack, isSubmitting }: Props) {
  ...
  // Wraps the engine's onSubmit: fires the owner notification ("visit" / new
  // lead) in parallel, without ever blocking or failing the actual test flow.
  const handleFormSubmit = async (data: FounderTestContactValues) => {
    notifyLead("visit", data);
    await onSubmit(data);
  };
      ...
      >
        <Box
          sx={{
            ...
            inset: 0,
            pointerEvents: "none",
            background: `radial-gradient(ellipse 70% 55% at 10% 40%, ${T.darkGlow1}, transparent),
            ...
            {INCLUSIONS.map((item, i) => (
              ...
              >
                <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
                  <CheckIcon
                    sx={{ fontSize: "1rem", color: "#818CF8", flexShrink: 0 }}
                  />
                  <Typography
                    sx={{
                      fontFamily: SANS,
          ...
          >
            ...
            >
              ...
              <Field
                ...
                error={!!errors.email}
                helperText={
                  errors.email?.message ?? "Your report will be delivered here"
                }
                disabled={isSubmitting}
                {...register("email")}
              />
