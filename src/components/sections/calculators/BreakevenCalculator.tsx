...
import { CalcLayout, T, FONT_MONO, FONT_SANS, FONT_SERIF } from './CalcLayout';
...
function computeBreakeven(inputs: {
  ...
  for (let m = 1; m <= months; m++) {
    ...
    forecast.push({ month: m, units: Math.round(units), revenue: Math.round(revenue), variableCost: Math.round(varCost), fixedCost: monthlyFixedCosts, profit: Math.round(profit), cumulativeProfit: Math.round(cumulative) });
  }
  return { contributionPerUnit, contributionMarginPct, breakevenUnits: isFinite(breakevenUnits) ? Math.ceil(breakevenUnits) : null, breakevenRevenue: isFinite(breakevenRevenue) ? Math.round(breakevenRevenue) : null, breakevenMonth, forecast };
}
        ...
        <Box>
          ...
          <ResponsiveContainer width="100%" height={260}>
            <AreaChart data={result.forecast} margin={{ top: 8, right: 4, left: 0, bottom: 0 }}>
              ...
              {result.breakevenMonth && (
                <ReferenceLine x={result.breakevenMonth} stroke={ACCENT} strokeDasharray="4 3" strokeWidth={1.5} label={{ value: `M${result.breakevenMonth}`, fill: ACCENT, fontSize: 10, fontFamily: FONT_MONO, fontWeight: 700 }} />
              )}
              <Area type="monotone" dataKey="cumulativeProfit" name="Cumulative Profit" stroke={ACCENT} strokeWidth={2} fill="url(#beProfGrad)" dot={false} activeDot={{ r: 4, fill: ACCENT, strokeWidth: 0 }} />
            </AreaChart>
          </ResponsiveContainer>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <ComposedChart data={result.forecast} margin={{ top: 8, right: 4, left: 0, bottom: 0 }}>
              ...
              <Bar dataKey="revenue" name="Revenue" fill={ACCENT + '44'} radius={[3,3,0,0]} barSize={10} />
